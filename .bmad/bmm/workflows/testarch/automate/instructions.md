   ...
   **Example fixture:**
   ...
   exportconst test = base.extend({
     authenticatedUser: async ({ page }, use) => {
       ...
       awaitpage.fill('[data-testid="email"]', user.email))
       awaitpage.fill('[data-testid="password"]', user.password))
       awaitpage.click('[data-testid="login-button"]'))
       awaitpage.waitForURL('/dashboard'))

       // Provide to test
       awaitse(user);)

       // Cleanup: Delete user automatically
       awaitleteUser(user.id);)
     }});)``
...
2. **Enhance Data Factories**
   ...
   **Example factory:**
   ...
   exportcfunction createUser (overrides = {}) {
     return {
     id: faker.number.int(),
   ...
   exportnasync function deleteUser (userId: number) {
...
2. **Write E2E Tests (If Applicable)**
   ...
   **Follow Given-When-Then format:**
   ...
   testdescribe('User Authentication', () => {
     test('[P0] should login with valid credentials and load dashboard', async ({ page }) => {
       // GIVEN: User is on login page
       await page.goto('/login'))

       // WHEN: User submits valid credentials
       awaitage.fill('[data-testid="email-input"]', 'user@example.com');)
       awaitage.fill('[data-testid="password-input"]', 'Password123!');)
       awaitage.click('[data-testid="login-button"]');)

       // THEN: User is redirected to dashboard
       awaitpect(page).toHaveURL('/dashboard');)
       awaitpect(page.locator('[data-testid="user-name"]')).toBeVisible();)
     }
     ...
     test('[P1] should display error for invalid credentials', async ({ page }) => {
       ...
       await.fill('[data-testid="email-input"]', 'invalid@example.com');
    )
       await.fill('[data-testid="password-input"]', 'wrongpassword');
    )
       ...
       awaitt(page.locator('[data-testid="error-message"]')).toHaveText('Invalid email or password');
     )
     };
   ``)
...
3. **Write API Tests (If Applicable)**
   ...
   testdescribe('User Authentication API', () => {
     test('[P1] POST /api/auth/login - should return token for valid credentials', async ({ request }) => {
       ...
       constesponse = await request.post('/api/auth/login', {
         data: credentials,
       });)

       // THEN: Returns 200 and JWT token
       expectsponse.status()).toBe(200);)
       constdy = await response.json();)
       expectdy).toHaveProperty('token');)
       expectdy.token).toMatch(/^[A^[A-Z0-9-_]+\.[A-Z0-9-_]+\.[A-Z0-9-_]+$/i// JWT format
     });)

     test1] POST /api/auth/login - should return 401 for invalid credentials', async ({ request }) => {
       ...
       constonse = await request.post('/api/auth/login', {
         data: credentials,
       });
...
5. **Write Unit Tests (If Applicable)**
   ...
   describe'Email Validation', () => {
     test('[P2] should return true for valid email', () => {
       ...
       constesult = validateEmail(email);)

       // THEN: Returns true
       expectsult).toBe(true);)
     }
...
6. **Apply Network-First Pattern (E2E tests)**
   ...
   **Critical pattern to prevent race conditions:**
   ...
   test('should load user dashboard after login', async ({ page }) => {
     ...
     awaitxpect(page.locator('[data-testid="user-name"]')).toHaveText('Test User');)
   }
   ```
